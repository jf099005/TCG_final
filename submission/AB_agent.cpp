...
const Move PASS_MOVE = Move(SQ_A1, SQ_H4);
...
int MoveOrderer::evaluate_move(const Position& pos, Move move){
    ...
    //positional score ~=(4~5)*piece_value ~= 200, capture score ~= 20, 

    int evaluation =  capture_score*4 + positional_score_after - positional_score_before;
    evaluation = evaluation*1000;
    if(move.type() == Moving){
        evaluation += history[piece.side][piece.type][move.from()][move.to()];
    }
    return evaluation;
}
...
void MoveOrderer::record_cut(Piece piece, Move move, int depth){
    history[piece.side][piece.type][move.from()][move.to()] += (depth > 0? depth:0);
...
Score ACDC::Negamax(Position pos, int depth, int remain_moves, Score alpha, Score beta, Move prv){
    ...
    if(critical_search){
        if(prv == PASS_MOVE){
            ...
            pos_copy.pass_turn();
            opt = -Negamax(pos_copy, depth-1, remain_moves-1, -beta, -alpha, PASS_MOVE);
            opt_move = PASS_MOVE;
            if(opt >= beta)
                return opt;
        }
    ...
    if(num_valid_moves == 0){
        ...
        pos.pass_turn();
        return -Negamax(pos, depth-1, remain_moves-1, -beta, -alpha, PASS_MOVE);
    }
    ...
    for(int move_idx = iteration_start; move_idx < num_valid_moves; move_idx++){
        ...
        //Scout search
        Score v = Star2_Evaluate(pos, nx_move, depth-1, remain_moves-1, -opt-EPS, -opt);
        if(v > opt){
            if(v >= beta){
                opt = v;
                opt_move = nx_move;
                ...
                v = Star2_Evaluate(pos, nx_move, depth-1, remain_moves-1, -beta, -alpha);
                opt = v;
                opt_move = nx_move;
            }
        }
        ...
        if(v >= beta){
            ...
            if(nx_move.type() == Moving){
                orderer->record_cut(pos.peek_piece_at(nx_move.from()),\
                        nx_move, depth);
            }
            #endif
            #ifdef TT_H
            if(depth > 0)
                TT->write(pos, depth, opt, opt_move, false);
            #endif
            correct_prediction += (move_idx == 0 && num_valid_moves > 1);
            fail_prediction += !(move_idx == 0 && num_valid_moves > 1);
            return v;
        }
        num_visited++;
    ...
    bool prediction = (num_valid_moves > 1 && opt_move == nx_moves[0]);
    
    correct_prediction += prediction;
    fail_prediction += !prediction;

    #ifdef HISTORY_HEURISTIC
    if(opt_move != PASS_MOVE && opt_move.type() == Moving){
